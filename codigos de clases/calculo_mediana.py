   ...
   if len(L) < 2001:
      ...
      for i in range(0,len(L)):
         if d <= L[i] and L[i] <= u:
            ...
            m_u = m_u + 1
      if m_d > len(L)//2 or m_u > len(L)//2 or len(S) > 4*math.floor(len(L)**(3/4)):
         return 
      else:
         S.sort()
...
if __name__ == "__main__":
    ...
    t3 = time.time()
    print("Largo de la lista: " + str(largo))
    print("Rango de los elementos en la lista: [-" + str(rango) + ", " + str(rango) + "]")
    print("Resultado algoritmo basada en ordenación: " + str(rM) + ", tiempo: " + str(t2 - t1) + " segundos")
    print("Resultado algoritmo aleatorizado: " + str(rMA) + ", tiempo: " + str(t3 - t2) + " segundos")
